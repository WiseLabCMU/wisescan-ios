...
enum AppConstants {
    ...
    static let coachRigGapSeconds: TimeInterval = 25                   // rig-mode coach: seconds of recording before the ceiling/floor mesh-gap prompts can fire (gives the sweep a fair chance first)
    static let coachFloorMinFaces = 1500                               // rig-mode coach: fewer floor-classified mesh faces than this late in a scan → "tilt the rig down" (floor gaps are the harder hole — nadir face is dropped downstream, so LiDAR is the only floor source)
    static let coachCeilingMinFaces = 500                              // rig-mode coach: fewer ceiling-classified faces → "tilt the rig up" (ceiling has image coverage via up-faces; mesh matters for the solver + mesh product)
