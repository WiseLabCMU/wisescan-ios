...
enum AppConstants {
    ...
    static let wearableFramesInFlight: Int = 1               // cap on concurrent wearable-frame decode/segment/encode jobs; excess frames are dropped, not queued — the work can outrun the stream and a queued frame is stale by the time it lands (the operator wants the newest proxy view, not a backlog)
