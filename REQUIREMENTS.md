...
| [DerivedSurfaces.swift](wisescan-ios/DerivedSurfaces.swift) | `derived_surfaces.json` sidecar: levels/ramps recovered from the classified mesh | `DerivedSurfacesData`, `Surface`, `schemaVersion`, `load(scanDirectory:)`, `matrix`, `centerY` |
...
| REQ-034 | Derived Levels & Ramps (Multi-Level Ghost Proxy) | ✅ **Implemented** — see REQ-034 below | — |
...
### REQ-033: 360° Still Sources
| | |
|:--|:--|
| **Status** | ✅ Complete (Theta X full; Z1 Wi-Fi-only; Insta360 X6 deferred-import path) |
| **Description** | A rig-mounted 360° source rides above the phone and contributes full-sphere imagery at each stillness pause, giving downstream reconstruction all-direction coverage the phone's frustum can't. **Ricoh Theta live mode** is BLE-first on the Theta X: pick the camera from a named Bluetooth list (advertised name = serial), one-time passkey bond, identity read → derived Wi-Fi credentials, wake-from-sleep over BLE (`cameraPower=on`), then a patient Wi-Fi join/probe (retries absorb the AP's 5–15 s rise and DHCP settle). A **multi-camera roster** switches bodies per collection (X for texture, Z1 for low light); Z1 connects Wi-Fi-only pending its BLE increment (its v1 GATT needs per-session UUID auth registered over Wi-Fi). At connect the camera's shooting state is **normalized** (image mode, self-timer off, running captures stopped) and zenith correction is enforced (leveling invariant). **The shutter rides BLE when the link is up** (~1.2–1.5 s faster per still; the new file's URL arrives as a NotifyState push — the download ticket with zero polling) with OSC fallback. **Deferred External Import mode** covers cameras such as the Insta360 X6 before SDK approval: capture writes the same per-still sidecars/tickets (phone pose + timestamps, no camera pose), then the operator imports stitched equirect JPEGs into the scan afterwards. Imports match by timestamp with a clock-offset estimate, warn on large mismatches, and use assumed-leveling provenance for Insta360 X6/X4/X3 faces until field validation is complete. In both modes, rig calibration solves per scan at Process time (EquirectPostCalibration, solver v7) against the completed mesh; the same privacy, face-export, and export pipeline consumes the resulting `equirect_stills/`. Tickets whose JPG never arrives are excluded from export. Security: live-camera originals are deleted after verified transfer (P1); imported stills never trigger camera-side deletion. Coloring can optionally draw exclusively from cube faces cut at baked poses (dev probe for pose quality). |
| **Source** | [StillSource.swift](wisescan-ios/StillSource.swift) — capture seam · [External360StillSource.swift](wisescan-ios/External360StillSource.swift) — deferred-import tickets + matching · [ThetaCameraManager.swift](wisescan-ios/ThetaCameraManager.swift) (+[+OSC](wisescan-ios/ThetaCameraManager+OSC.swift), [+ScanCapture](wisescan-ios/ThetaCameraManager+ScanCapture.swift)) — live Theta connect/roster/shutter/downloads · [ThetaBLEManager.swift](wisescan-ios/ThetaBLEManager.swift) (+[Link](wisescan-ios/ThetaBLEManager+Link.swift)/[Delegates](wisescan-ios/ThetaBLEManager+Delegates.swift)) — production BLE · [ThetaBLEProbe.swift](wisescan-ios/ThetaBLEProbe.swift) — dev GATT bench · [EquirectPostCalibration.swift](wisescan-ios/EquirectPostCalibration.swift) + [RigCalibrationSolver.swift](wisescan-ios/RigCalibrationSolver.swift) — per-scan solve · [EquirectFaceExport.swift](wisescan-ios/EquirectFaceExport.swift) + [EquirectGPU.swift](wisescan-ios/EquirectGPU.swift) — cube faces · [DashboardView.swift](wisescan-ios/DashboardView.swift) — source picker + camera cards |
...
### REQ-034: Derived Levels & Ramps (Multi-Level Ghost Proxy)
| | |
|:--|:--|
| **Status** | ✅ Complete (derivation, sidecar, semantic preview; vertical-axis registration deliberately out of scope) |
| **Description** | RoomPlan models **one floor per room**, so a step, a landing, a mezzanine, or a shallow ramp arrives as bare mesh with no plane to represent it — and the ghost proxy, which subtracts modelled quads from the mesh it bakes, had nothing to put there. `ARCoverageView.buildGhostProxyOBJ` (the Process step's PROXY stage) now re-reads the scan's **own classified mesh** — `mesh.obj` parsed by `MeshParser.parseOBJRaw` plus `face_classes.bin`, with a face-for-face alignment contract that refuses the build on a count mismatch — and recovers what RoomPlan missed. **(1) Levels.** Floor-class faces vote into a 5 cm height histogram (only near-level faces vote, `levelFaceMaxPitchDeg` 10°); a candidate must gather `levelMinAreaM2` (1.0 m²) inside a ±6 cm slab, keep an area-weighted mean tilt under `levelMeanTiltMaxDeg` (2.5°, the flat-surface-vs-slice-of-a-ramp test, tighter than the 4.76° ADA ramp maximum), span at least `levelMinSpanMeters` (0.6 m) in plan view, and clear other accepted levels by `levelSeparationMeters` (0.10 m) — enough to resolve a standard 17–18 cm riser into two levels while keeping a staircase from registering tread-by-tread (treads are excluded upstream by pitch, span and area). A level's extent is the bounding box of the faces that actually contributed per connected component, not the room footprint. **(2) Ramps.** Coherently sloped floor patches not already explained by an accepted level are fitted as `ramp` planes, gated on `rampMinAreaM2` (1.5 m²), plane RMS ≤ `rampMaxResidualMeters` (6 cm), tilt ≤ `rampMaxTiltDeg` (15° — comfortably under a staircase's 30–37° effective slope, so a flight is not a ramp either), and `rampMinFillRatio` (0.6) of their own bounding rectangle, which is the explicit "is this actually quad-like?" test that declines a curve. Fitted ramps are extended to meet the levels they run into. Both derivations return the rejected candidates with verdicts, for the build log. **(3) Categories are `level` / `ramp`, never `floor`.** `PlaneRegistration.plane(category:)` decodes only `"wall"` and `"floor"` and returns nil for these, so a decoder pointed at the sidecar drops these surfaces on the floor rather than feeding them silently into a solve — the fit's plane matcher admits pairs up to 25° apart, so a shallow ramp would happily correspond to a flat floor and pull the vertical solution with it. A guard test asserts the refusal. **Vertical-axis alignment is not wired up by this work**; anything consuming these for a fit has to opt in per category. **(4) `derived_surfaces.json`.** Written by `ScanPostprocessor` in the same step and from the same inputs as `mesh_proxy.obj`, so it is co-framed with the mesh and the proxy by construction and must be **regenerated rather than transformed** whenever they are; its staleness rides `ghostProxyVersionHeader` (currently `# ghostproxy v24`) rather than a header of its own, while `load` independently rejects any `version` other than the current `schemaVersion` — checked per URL, so a stale top-level copy falls through to a current `raw_data/` mirror instead of shadowing it. A rebuild that finds nothing **deletes** the file rather than leaving the previous run's answer standing, and the sidecar is cleared before a re-framed rebuild so a raw-frame artifact cannot survive into a canonical-frame scan. Written to the scan directory and mirrored to `raw_data/`; it is an **on-device artifact and is not included in any export archive**. **(5) Cell-backed quads.** Every quad's rectangle is rasterised onto a 0.25 m support grid (`QuadSupport`, deliberately decoupled from the 1 m bake lattice), and a cell counts as backed only when the class-matched mesh nearest it — assigned by competitive best fit, not any-plane-within-tolerance — has an area-weighted mean normal agreeing with the plane (`quadSupportFloorMeanTiltDeg` 3°, `quadSupportWallMeanTiltDeg` 20°). Bake and subtraction read the **same** mask, so a quad draws and subtracts only where mesh demonstrably backs it and a dropped cell can never leave a hole; `quadSupportDilateCells` is the thin-scanning allowance, and a border flood reports surviving interior pinholes as an objective patchiness metric. **(6) Quad demotion.** Each RoomPlan quad scores explained-area / present-area inside its own footprint; below `quadModelMinExplainedRatio` (0.5) it is **demoted** — it neither subtracts nor bakes, and its mesh stays — because a chord across a curve explains only its tangency strip while a real straight wall explains nearly everything. Presence is orientation-gated at `quadModelPresenceTiltDeg` (55°) so the perpendicular wall crossing every corner cannot pollute a straight wall's denominator. Derived levels and ramps are **exempt by construction**: they are measured from this mesh, so they cannot mis-model it. **(7) Semantic preview.** `MeshPreviewView.buildRoomPlanOutlines` draws derived surfaces in the existing floor class (legend and palette untouched) and **skips RoomPlan's own floor** when a derived level sits at the same height, since the derived extent measures the same physical surface better. Because the sidecar carries only `dimensions` + `transform`, the preview draws each surface as its **full oriented rectangle** — the per-cell support mask that trims the proxy lives only inside `mesh_proxy.obj` — so a sparsely-filled level renders as its whole bounding box. **(8) Plane-anchor orientation gate.** `PlaneRegistration.planes(fromPlaneAnchors:)` now rejects a live anchor whose normal disagrees with its own category beyond `anchorOrientationToleranceDeg` (15°), because ARKit can hand a ramp back classified `.floor` and a tilted "floor" corrupts the only constraint the vertical translation has. **(9) Force Rebuild Artifacts.** A Developer Mode toggle re-queues the `.proxy` step — proxy, dynamic mesh and derived surfaces, which are built together — on an already-current scan, because the builders compute their diagnostics *during* the build and a version-current scan is otherwise un-inspectable without bumping a version header. Registration is deliberately never forced: it bakes a transform into `mesh.obj` in place, so re-running would double-bake. |
| **Source** | [ARCoverageView.swift](wisescan-ios/ARCoverageView.swift) — `buildGhostProxyOBJ`, `deriveLevelPlanes` / `LevelDerivation`, `deriveRampPlanes` / `RampDerivation`, `classifyPatches`, `QuadSupport` + `buildQuadSupport` / `quadCoverage` / `quadCovers`, `quadModelFitness`, `ghostProxyVersionHeader`, and the `level*` / `ramp*` / `quadModel*` / `quadSupport*` tuning constants · [DerivedSurfaces.swift](wisescan-ios/DerivedSurfaces.swift) — `DerivedSurfacesData`, `Surface`, `load(scanDirectory:)` (top-level copy preferred over the `raw_data/` mirror, each version-checked), `matrix`, `centerY` · [ScanPostprocessor.swift](wisescan-ios/ScanPostprocessor.swift) — PROXY-step sidecar write/remove, `proxyIsCurrent`, `forceRebuild` plumbing through `pendingSteps` · [MeshPreviewView.swift](wisescan-ios/MeshPreviewView.swift) — `buildRoomPlanOutlines` derived-surface rendering + same-height RoomPlan-floor dedupe · [PlaneRegistration.swift](wisescan-ios/PlaneRegistration.swift) — `orientationMatches`, `anchorOrientationToleranceDeg`, wall/floor-only category decode · [MeshParser.swift](wisescan-ios/MeshParser.swift) — `parseOBJRaw` · [SettingsView.swift](wisescan-ios/SettingsView.swift) — Force Rebuild Artifacts toggle · [ScansListView.swift](wisescan-ios/ScansListView.swift) — `reRunProcessing()` · [AppConstants.swift](wisescan-ios/AppConstants.swift) — `Key.forceRebuildArtifacts`, `forceRebuildArtifacts` default · [ScanStore.swift](wisescan-ios/ScanStore.swift) — `raw_data/` → scan-dir promotion list · [schemas/derived_surfaces.schema.json](schemas/derived_surfaces.schema.json) — sidecar schema |
| **Notes** | Extends REQ-032's PROXY stage; the sidecar is on-device only (not in any export bundle). A staircase **flight** produces neither levels nor ramps by design — treads fail level pitch/span/area, and a ~30° flight exceeds the 15° ramp tilt cap; what resolves is the single riser between two walkable surfaces. Deliberately deferred: feeding levels/ramps into the vertical-axis fit, and trimming the semantic preview to the per-cell support mask. |
