...
| **Privacy Filter** | Person segmentation removes humans from mesh; face detection blurs faces on camera feed and in exports | ✅ Complete |
| **3D Scan Preview** | Interactive SceneKit preview with height-based coloring on Workflows tab | ✅ Complete |
| **Export Formats** | OBJ, PLY, USDZ, and RAW (Nerfstudio-compatible) | ✅ Complete |
...
| **Capture Settings** | Image overlap maximum, motion blur rejection for RAW exports | ✅ Complete |
...
├── DashboardView.swift        # Server connection config, upload URL
├── CaptureView.swift          # Live capture UI, recording controls, scan HUD
├── ARCoverageView.swift       # ARKit scene reconstruction, person segmentation, OBJ export
├── FaceBlurOverlay.swift      # Live face detection overlay + face blur utility for exports
├── FrameCaptureSession.swift  # RAW data capture (RGB, depth, poses → transforms.json)
├── WorkflowsView.swift        # Scan cards, format picker, save/upload actions
├── MeshPreviewView.swift      # SceneKit 3D mesh preview with height coloring
├── ScanStore.swift            # Shared data models (CapturedScan, ExportFormat, ScanStats)
└── SettingsView.swift         # RAW export settings, workflow guide, recommended apps
```
...
4. Configure the upload URL on the Dashboard tab
